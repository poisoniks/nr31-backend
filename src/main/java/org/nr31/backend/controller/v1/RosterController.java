...
public class RosterController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved unit types", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = UnitTypeDTO.class))))
    })
    @GetMapping(value = "/unit-types", produces = "application/json")
    public ResponseEntity<List<UnitTypeDTO>> getAllUnitTypes() {
        return ResponseEntity.ok(rosterService.getAllUnitTypes());
    }
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved event types", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = EventTypeDTO.class))))
    })
    @GetMapping(value = "/event-types", produces = "application/json")
    public ResponseEntity<List<EventTypeDTO>> getAllEventTypes() {
        return ResponseEntity.ok(rosterService.getAllEventTypes());
    }
